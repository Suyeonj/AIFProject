...
def train(args, data, show_loss, show_topk, page_repl):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for step in range(args.n_epochs):
            ...
            # top-K evaluation
            if show_topk:
                precision, recall, hit_lists = topk_eval(sess, model, user_list, train_record, test_record, item_set, k_list, args.batch_size)
                print('precision: ')
                for i in precision:
                    print('%.4f\t' % i)
                    print('  ')
...
def topk_eval(sess, model, user_list, train_record, test_record, item_set, k_list, batch_size):
    ...
    for user in user_list:
        ...
        hit_lists = []
        for k in k_list:
            hit_list = [0 for i in range(k)]
            for k_idx in range(k):
                if (set(item_sorted[:k])[k_idx] == test_record[user][k_idx]):
                    hit_list[k_idx] = 1
    ...
    return precision, recall, hit_lists
